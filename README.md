...
│   ├── milestone2_rag_eval.py   # Milestone 2: hybrid RAG JSON for ``results/milestone2_rag_eval_runs.json``
...
This runs **two** steps: (1) BM25 vs semantic comparison for every query in `data/processed/ground_truth.csv` → `data/processed/qualitative_eval_runs.csv`; (2) Milestone 2 **hybrid RAG** on a fixed 10-query set → `results/milestone2_rag_eval_runs.json` (requires **`GROQ_API_KEY`** in `.env`). Milestone 1 discussion: `results/milestone1_discussion.md`; Milestone 2 RAG notes: `results/milestone2_discussion.md`.

To run only the Milestone 1 CSV or only the Milestone 2 JSON:
...
make eval      # qualitative CSV + Milestone 2 RAG JSON (needs GROQ_API_KEY for JSON)
